...
Handlebars.registerHelper('generateDates', function (dates) {
    ...
    function convertIntoRange(dateArray, type, hidden) {
        var dateRange = false;
        html += `<div role="tabpanel" id="qld__service_dates__tab-panel--${type}" aria-labelledby="qld__service_dates__tab-heading--${type}" class="qld__tab_panel mt-1" ${hidden}>`;
...
Handlebars.registerHelper('listAZ', function(items, url, options) {
    ...
    for(var i = 0; i < letters.length; i++) {
        ...
        html += '<h3 class="qld__services_a-z__list__item__header"><span id="'+letters[i]+'">'+letters[i]+'</span></h3>';
        html += '<ul class="row qld__services_a-z__list__item__services">';
        for(var k = 0; k < services[letters[i]].length; k++) {
            html += '<li class="col-lg-4 col-md-6 col-xs-12 qld__services_a-z__list__item__services__item"><a class="qld__services_a-z__list__item__services__item__link" href="'+url+'/'+services[letters[i]][k].web_path+'"><span>'+services[letters[i]][k].name+'</span><i class="fas fa-arrow-right"></i></a></li>';
        }
        html += '</ul>';
    }
